...
const SelectGameCardContainer: React.FC<SelectGameCardContainerProps> = ({ game, handleAddBet, allBetsForWeek, currentUserId }) => {
    ...
    const handleClickOpen = () => {
        ...
        const totalBetsForThisGame = allBetsForWeek.flatMap(userBets => userBets.bets)
          .filter(bet => bet.game.gameId === game.gameId)
          .length;
      
        // Count the bets placed by the current user this week
        const userBetsThisWeek = allBetsForWeek
          .find(userBets => userBets.uid === currentUserId)?.bets.length || 0;
    ...
    return (
      <div className={shouldShake ? "subtle-shake" : ""}>
        ...
        <Dialog onClose={handleClose} open={open}>
          ...
          <SelectGameCardDialog game={game} handleAddBet={handleCloseAndAdd} allBetsForWeek={allBetsForWeek} />
        </Dialog>
      </div>
    );
  };
