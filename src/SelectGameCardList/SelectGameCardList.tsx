...
import { AlertProps, Bet, Game, convertToLocalTime } from "../Utils/Utils";
...
const SelectGameCardList: React.FC<SelectGameCardListProps> = ({
    game,
    filterText,
    handleAddBet,
    allBetsForWeek,
    ...
    const groupedByDate = sortedGameSelections.reduce(
        (acc: { [date: string]: Game[] }, currentGame) => {
            const localDate = currentGame.date; // Since it's already in local timezone
            const dateKey = `${localDate.getFullYear()}-${String(localDate.getMonth() + 1).padStart(
                2,
                "0"
            )}-${String(localDate.getDate()).padStart(2, "0")}`; // Format YYYY-MM-DD
            if (!acc[dateKey]) {
                acc[dateKey] = [];
            }
